...
:CPageFloat(pRect, pParent)//,m_Mutex(MUTEX_RECURSIVE)//csp modify 20121118
{
	...
	for (i=0; i<25; ++i)//24 hour
	{
		...
		if (i<10) //一位数
		{
			pTextHour[i] = CreateStatic(CRect(rtTmp.left+i*interval-TEXT_HEIGHT/4, rtTmp.top, rtTmp.left+i*interval+TEXT_HEIGHT/4, rtTmp.top+TEXT_HEIGHT), this, str_num);
		}
		else if (i<24)//两位数
		{
			pTextHour[i] = CreateStatic(CRect(rtTmp.left+i*interval-TEXT_HEIGHT/2, rtTmp.top, rtTmp.left+i*interval+TEXT_HEIGHT/2, rtTmp.top+TEXT_HEIGHT), this, str_num);
		}
		else //最后一个偏左一点
		{
			pTextHour[i] = CreateStatic(CRect(rtTmp.left+i*interval-TEXT_HEIGHT/2-4, rtTmp.top, rtTmp.left+i*interval+TEXT_HEIGHT/2-4, rtTmp.top+TEXT_HEIGHT), this, str_num);
		}
	...
	for (i=0; i<4; ++i)
	{
		...
		pButton[PB_BUTTON_24HR+i]->SetBitmap(VD_LoadBitmap(DATA_DIR"/temp/btn.bmp"), VD_LoadBitmap(DATA_DIR"/temp/btn_f.bmp"), VD_LoadBitmap(DATA_DIR"/temp/btn_f.bmp"));
	}
#if 0	
	rtTmp.left = 41;
	rtTmp.top += 32+32;
